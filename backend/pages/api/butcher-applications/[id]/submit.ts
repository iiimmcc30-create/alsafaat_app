// pages/api/butcher-applications/[id]/submit.ts
...
import { submitBodySchema } from '@/butcher-applications/routes/schemas';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  return withAuth(req, res, submitApplication);
}

async function submitApplication(req: AuthedRequest, res: NextApiResponse) {
  ...
  try {
    const application = await butcherApplicationUserService.submitApplication(
      req.user.userId,
      applicationId,
      parsed.data,
    );
